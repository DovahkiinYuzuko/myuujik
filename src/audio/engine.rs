...
impl AudioEngine {
    ...
    pub fn set_normalize_loudness(&self, enabled: bool) {
        ...
        loop {
            ...
            loop {
                ...
                match cmd {
                        ...
                        if preloaded.as_ref().map(|(p, _)| p) == Some(&path) {
                            ...
                            match AudioDecoder::open(&path) {
                                Ok(dec) => {
                                    ...
                                    next_replay_gain = if normalize_loudness {
                                        dec.metadata().replay_gain_db.map(|db| 10.0f32.powf(db / 20.0)).unwrap_or(1.0)
                                    } else {
                                        1.0
                                    };
                            ...
                            Ok(dec) => {
                                ...
                                match b_res {
                                    Ok((mut b, used_prod)) => {
                                        ...
                                        current_replay_gain = if normalize_loudness {
                                            meta.replay_gain_db.map(|db| 10.0f32.powf(db / 20.0)).unwrap_or(1.0)
                                        } else {
                                            1.0
                                        };
                    ...
                    EngineCommand::SetNormalizeLoudness(enabled) => {
                        normalize_loudness = enabled;
